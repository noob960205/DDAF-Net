...
class BaseDataset(Dataset):
    ...
    def load_multi_image(self, i: int, rect_mode: bool = True) -> Tuple[np.ndarray, Tuple[int, int], Tuple[int, int]]:
        """
        Load RGB and IR images from dataset index 'i' and combine them into a 6-channel image.
        ...
        Returns:
            im (np.ndarray): Loaded 6-channel image as a NumPy array (RGB + IR).
            hw_original (Tuple[int, int]): Original image dimensions in (height, width) format.
            hw_resized (Tuple[int, int]): Resized image dimensions in (height, width) format.

        Raises:
            FileNotFoundError: If the RGB or IR image file is not found.
        """
        im, f_rgb, fn = self.ims[i], self.im_files[i], self.npy_files[i]
        f_ir = self.im_files[i].replace('imageRGB', 'imageIR')
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                if im_ir is None:
                    raise FileNotFoundError(f"IR Image Not Found {f_ir}")
                # TODO check (H_rgb, H_ir) & (W_rgb, W_ir)
                # Combine RGB and IR images
                im = np.dstack((im_rgb, im_ir))
            if im is None:
                raise FileNotFoundError(f"Combined Image Not Found {f_rgb} or {f_ir}")
    ...
    def get_image_and_label(self, index: int) -> Dict[str, Any]:
        ...
        if self.channels == 3:
            ...
            label["img"], label["ori_shape"], label["resized_shape"] = self.load_multi_image(index)
        else:
            raise ValueError(f"Unsupported number of channels: {self.channels}. Expected 3 or 6.")
