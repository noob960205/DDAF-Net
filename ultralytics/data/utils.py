...
def img2label_paths(img_paths: List[str]) -> List[str]:
    ...
    This function intelligently determines the image directory pattern (e.g., 'images',
    'imageIR', 'imageRGB') from the first path in the list and applies this pattern
    to convert all paths to their corresponding label paths.

    Args:
        img_paths (List[str]): A list of paths to image files. All paths are expected
                               to follow the same directory structure.

    Returns:
        List[str]: A list of corresponding paths to label files.

    Raises:
        ValueError: If no valid image directory is found in the first image path.
    """
    # 如果路径列表为空，直接返回空列表
    if not img_paths:
        return []

    # 定义可能的图像目录（按优先级）和固定的标签目录，一般使用IR模态的标签
    search_dirs = [f'{os.sep}images{os.sep}', f'{os.sep}imageIR{os.sep}', f'{os.sep}imageRGB{os.sep}']
    label_dir = f'{os.sep}labels{os.sep}'

    # 根据列表中的第一个路径来确定替换规则（sa, sb）
    first_path = img_paths[0]
    sa = None
    for s_dir in search_dirs:
        if s_dir in first_path:
            sa = s_dir
            break

    # 如果在第一个路径中没有找到任何一个有效的图像目录，则抛出错误
    if sa is None:
        raise ValueError(f"Could not determine image-to-label mapping. "
                         f"The first image path '{first_path}' does not contain any of "
                         f"the expected directories: {search_dirs}")

    # 确定替换规则后，使用列表推导式高效地转换所有路径，这与原始代码的风格和效率一致
    sb = label_dir
    return [sb.join(x.rsplit(sa, 1)).rsplit(".", 1)[0] + ".txt" for x in img_paths]
